...
import { Calendar, Clock, TrendingUp, Sparkles, User, ChevronRight, FileText, Share2 } from 'lucide-react';
...
const Dashboard: React.FC<DashboardProps> = ({ records, employee }) => {
  ...
  const handleShare = async () => {
    if (navigator.share) {
      ...
      alert("¡Enlace copiado al portapapeles para enviar!");
    }
  };
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100 flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <button 
          onClick={handleShare}
          className="bg-indigo-50 text-indigo-600 px-6 py-3 rounded-2xl font-bold text-sm flex items-center hover:bg-indigo-100 transition-all w-full md:w-auto justify-center"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
          <div className="flex justify-between items-start mb-4">
            <div className="p-2 bg-green-50 rounded-xl"><TrendingUp size={24} className="text-green-600" /></div>
          </div>
        ...
        <div className="bg-white p-6 rounded-3xl shadow-sm border border-slate-100">
          <div className="flex justify-between items-start mb-4">
            <div className="p-2 bg-amber-50 rounded-xl"><Calendar size={24} className="text-amber-600" /></div>
          </div>
          ...
          {aiReport ? (
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-6">
              <p className="text-slate-400 max-w-md">
                Deja que nuestra IA analice tus patrones de trabajo para darte consejos sobre productividad.
              </p>
              <button 
              ...
              >
                {isGenerating ? "Procesando..." : "Analizar con IA"}
              </button>
            </div>
          )}
        </div>
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <div className="divide-y divide-slate-50">
          {records.length === 0 ? (
            <div className="p-10 text-center text-slate-400">No hay registros hoy</div>
          ) : (
              ...
              <div key={record.id} className="p-4 flex items-center justify-between hover:bg-slate-50 transition-colors">
                ...
                <div className="text-right">
                  <span className={`px-3 py-1 rounded-full text-xs font-bold ${record.checkOut ? 'bg-slate-100 text-slate-600' : 'bg-green-100 text-green-600'}`}>
                    {record.checkOut ? 'Cerrado' : 'Abierto'}
                  </span>
                </div>
              </div>
            ))
